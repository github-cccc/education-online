package com.githubzhu.member.service

import com.githubzhu.member.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.githubzhu.member.dao.DwdMemberDao
...
//        val domain = dwsMembers.map(_.domain).head
  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    ...
    /* import sparkSession.implicits._
     val dwdMember: Dataset[Row] = DwdMemberDao.getDwdMember(sparkSession).where(s"dt=${dt}") //主表用户
   ...
   def importMember(sparkSession: SparkSession, time: String) = {
     ...
     dayResult.union(historyResult).groupByKey(item => item.uid + "_" + item.dn)
       .mapGroups { case (key, iters) =>
         ...
         if (list.size > 1 && "9999-12-31".equals(list(list.size - 2).end_time)) {
           //如果存在历史数据，需要对历史数据end_time 进行修改
           //获取历史数据的最后一条
           val oldLastMode = list(list.size - 2)
           //获取当前时间最后一条数据
           val LastMode = list(list.size - 1)

           oldLastMode.end_time = LastMode.start_time
           LastMode.paymoney= (BigDecimal.apply(LastMode.paymoney)+BigDecimal(oldLastMode.paymoney).toString())
         }
           MemberZipperResult(list)
